...
load_geometries().then(() => {
    ...
    //Kotitehtävä: Lisää loput nivelet
...
mqtt_client.on('message', (topic, message) => {
    if (joints.length == 6){
        ...
        joints[4].rotation.set(THREE.Math.degToRad(joint_data.joints[3]) - THREE.Math.degToRad(joint_data.joints[2]), 0, 0);
        joints[5].rotation.set(0, 0, THREE.Math.degToRad(joint_data.joints[4]) - THREE.Math.degToRad(joint_data.joints[3]));
        joints[6].rotation.set(THREE.Math.degToRad(joint_data.joints[5]) - THREE.Math.degToRad(joint_data.joints[4]), 0, 0);
    }
